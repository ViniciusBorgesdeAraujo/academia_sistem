 ...
 */
class documentos extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'documentos';
		$this->TableName = 'documentos';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`documentos`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// iddocumentos
		$this->iddocumentos = new DbField('documentos', 'documentos', 'x_iddocumentos', 'iddocumentos', '`iddocumentos`', '`iddocumentos`', 3, 11, -1, FALSE, '`iddocumentos`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->iddocumentos->IsAutoIncrement = TRUE; // Autoincrement field
		$this->iddocumentos->IsPrimaryKey = TRUE; // Primary key field
		$this->iddocumentos->Sortable = TRUE; // Allow sort
		$this->iddocumentos->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
		$this->fields['iddocumentos'] = &$this->iddocumentos;

		// idpessoa
		$this->idpessoa = new DbField('documentos', 'documentos', 'x_idpessoa', 'idpessoa', '`idpessoa`', '`idpessoa`', 3, 11, -1, FALSE, '`idpessoa`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// tipo
		$this->tipo = new DbField('documentos', 'documentos', 'x_tipo', 'tipo', '`tipo`', '`tipo`', 3, 11, -1, FALSE, '`tipo`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		switch ($CurrentLanguage) {
			case "pt-BR":
				$this->tipo->Lookup = new Lookup('tipo', 'tipodocumentos', FALSE, 'idtipodocumentos', ["Tipo","","",""], [], [], [], [], [], [], '', '');
				break;
			default:
				$this->tipo->Lookup = new Lookup('tipo', 'tipodocumentos', FALSE, 'idtipodocumentos', ["Tipo","","",""], [], [], [], [], [], [], '', '');
				break;
		...
		// numero
		$this->numero = new DbField('documentos', 'documentos', 'x_numero', 'numero', '`numero`', '`numero`', 200, 45, -1, FALSE, '`numero`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`documentos`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->iddocumentos->setDbValue($conn->insert_ID());
			$rs['iddocumentos'] = $this->iddocumentos->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('iddocumentos', $rs))
				AddFilter($where, QuotedName('iddocumentos', $this->Dbid) . '=' . QuotedValue($rs['iddocumentos'], $this->iddocumentos->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	protected function loadDbValues(&$rs)
	{
		...
		$this->iddocumentos->DbValue = $row['iddocumentos'];
	...
	{
		return "`iddocumentos` = @iddocumentos@";
	}
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('iddocumentos', $row) ? $row['iddocumentos'] : NULL;
		else
			$val = $this->iddocumentos->OldValue !== NULL ? $this->iddocumentos->OldValue : $this->iddocumentos->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@iddocumentos@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	{
		global $Language;
		if ($pageName == "documentosview.php")
			return $Language->phrase("View");
		elseif ($pageName == "documentosedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "documentosadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("documentosview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("documentosview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "documentosadd.php?" . $this->getUrlParm($parm);
		else
			$url = "documentosadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("documentosedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("documentosadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("documentosdelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "iddocumentos:" . JsonEncode($this->iddocumentos->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->iddocumentos->CurrentValue != NULL) {
			$url .= "iddocumentos=" . urlencode($this->iddocumentos->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("iddocumentos") !== NULL)
				$arKeys[] = Param("iddocumentos");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->iddocumentos->CurrentValue = $key;
			else
				$this->iddocumentos->OldValue = $key;
	...
	{
		$this->iddocumentos->setDbValue($rs->fields('iddocumentos'));
	...
	public function renderListRow()
	{
		...
		// iddocumentos
		$this->iddocumentos->LinkCustomAttributes = "";
		$this->iddocumentos->HrefValue = "";
		$this->iddocumentos->TooltipValue = "";
	...
	public function renderEditRow()
	{
		...
		// iddocumentos
		$this->iddocumentos->EditAttrs["class"] = "form-control";
		$this->iddocumentos->EditCustomAttributes = "";
		$this->iddocumentos->EditValue = $this->iddocumentos->CurrentValue;
		$this->iddocumentos->ViewCustomAttributes = "";
		...
		if (!$doc->ExportCustom) {
			...
			if ($doc->Horizontal) { // Horizontal format, write header
				...
				if ($exportPageType == "view") {
					$doc->exportCaption($this->iddocumentos);
		...
		while (!$recordset->EOF && $recCnt < $stopRec) {
			...
			if ($recCnt >= $startRec) {
				...
				if (!$doc->ExportCustom) {
					...
					if ($exportPageType == "view") {
						$doc->exportField($this->iddocumentos);
	...
	public function getUserIDSubquery(&$fld, &$masterfld)
	{
		global $UserTable;
		$wrk = "";
		$sql = "SELECT " . $masterfld->Expression . " FROM `documentos`";
	...
	function Row_Updating($rsold, &$rsnew) {
		...
		$aulas=$rsnew["idaulas"];
		$aluno=$rsold["idpessoa"];
	Execute("UPDATE aulas SET idaluno='".$aluno."' where idaulas='".$aulas."'");
		return TRUE;
	}
