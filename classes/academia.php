...
/**
 * Table class for academia
 */
class academia extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 'academia';
		$this->TableName = 'academia';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`academia`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// idacademia
		$this->idacademia = new DbField('academia', 'academia', 'x_idacademia', 'idacademia', '`idacademia`', '`idacademia`', 3, 11, -1, FALSE, '`idacademia`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->idacademia->IsAutoIncrement = TRUE; // Autoincrement field
		$this->idacademia->IsPrimaryKey = TRUE; // Primary key field
		...
		// nome
		$this->nome = new DbField('academia', 'academia', 'x_nome', 'nome', '`nome`', '`nome`', 200, 45, -1, FALSE, '`nome`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nome->Sortable = TRUE; // Allow sort
		$this->fields['nome'] = &$this->nome;

		// registro
		$this->registro = new DbField('academia', 'academia', 'x_registro', 'registro', '`registro`', '`registro`', 200, 45, -1, FALSE, '`registro`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->registro->Sortable = TRUE; // Allow sort
		$this->fields['registro'] = &$this->registro;

		// ativado
		$this->ativado = new DbField('academia', 'academia', 'x_ativado', 'ativado', '`ativado`', '`ativado`', 3, 11, -1, FALSE, '`ativado`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
		$this->ativado->Sortable = TRUE; // Allow sort
		switch ($CurrentLanguage) {
			case "pt-BR":
				$this->ativado->Lookup = new Lookup('ativado', 'academia', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
				break;
			default:
				$this->ativado->Lookup = new Lookup('ativado', 'academia', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
				break;
		...
		// idaluno
		$this->idaluno = new DbField('academia', 'academia', 'x_idaluno', 'idaluno', '`idaluno`', '`idaluno`', 3, 11, -1, FALSE, '`idaluno`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	public function getDetailUrl()
	{
		...
		// Detail url
		$detailUrl = "";
		if ($this->getCurrentDetailTable() == "turnos") {
			$detailUrl = $GLOBALS["turnos"]->getListUrl() . "?" . Config("TABLE_SHOW_MASTER") . "=" . $this->TableVar;
			$detailUrl .= "&fk_idacademia=" . urlencode($this->idacademia->CurrentValue);
		}
	...
	{
		return ($this->SqlFrom != "") ? $this->SqlFrom : "`academia`";
	}
	...
	public function insert(&$rs)
	{
		...
		if ($success) {
			...
			$this->idacademia->setDbValue($conn->insert_ID());
			$rs['idacademia'] = $this->idacademia->DbValue;
		}
		return $success;
	}
	...
	protected function deleteSql(&$rs, $where = "", $curfilter = TRUE)
	{
		...
		if (is_array($where))
			$where = $this->arrayToFilter($where);
		if ($rs) {
			if (array_key_exists('idacademia', $rs))
				AddFilter($where, QuotedName('idacademia', $this->Dbid) . '=' . QuotedValue($rs['idacademia'], $this->idacademia->DataType, $this->Dbid));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		AddFilter($filter, $where);
		if ($filter != "")
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('idacademia', $row) ? $row['idacademia'] : NULL;
		else
			$val = $this->idacademia->OldValue !== NULL ? $this->idacademia->OldValue : $this->idacademia->CurrentValue;
		if (!is_numeric($val))
			return "0=1"; // Invalid key
		if ($val == NULL)
			return "0=1"; // Invalid key
		else
			$keyFilter = str_replace("@idacademia@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
		return $keyFilter;
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "academiaedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "academiaadd.php")
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("academiaview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("academiaview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
	...
	{
		if ($parm != "")
			$url = "academiaadd.php?" . $this->getUrlParm($parm);
		else
			$url = "academiaadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("academiaedit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("academiaedit.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm != "")
			$url = $this->keyUrl("academiaadd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("academiaadd.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("academiadelete.php", $this->getUrlParm());
	}
	...
	{
		$json = "";
		$json .= "idacademia:" . JsonEncode($this->idacademia->CurrentValue, "number");
		$json = "{" . $json . "}";
		if ($htmlEncode)
	...
	public function keyUrl($url, $parm = "")
	{
		...
		if ($parm != "")
			$url .= $parm . "&";
		if ($this->idacademia->CurrentValue != NULL) {
			$url .= "idacademia=" . urlencode($this->idacademia->CurrentValue);
		} else {
	...
	public function getRecordKeys()
	{
		...
		if (Param("key_m") !== NULL) {
			...
			if (Param("idacademia") !== NULL)
				$arKeys[] = Param("idacademia");
	...
	public function getFilterFromRecordKeys($setCurrent = TRUE)
	{
		...
		foreach ($arKeys as $key) {
			...
			if ($setCurrent)
				$this->idacademia->CurrentValue = $key;
			else
				$this->idacademia->OldValue = $key;
	...
	public function loadListRowValues(&$rs)
	{
		...
		$this->registro->setDbValue($rs->fields('registro'));
	...
	public function renderEditRow()
	{
		...
		$this->idacademia->EditValue = $this->idacademia->CurrentValue;
		$this->idacademia->ViewCustomAttributes = "";
